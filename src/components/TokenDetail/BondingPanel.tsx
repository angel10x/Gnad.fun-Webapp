    ...
    <div className="p-4 space-y-4">
      ...
      <div className='bg-white/10 p-4 rounded-md bg-gray-900'>
        <div className="grid grid-cols-2 mb-4">
          <button onClick={() => setMode('buy')} className={`${mode === 'buy' ? 'bg-btn-buy text-black' : 'bg-gray-900 text-white'} py-2 rounded-md cursor-pointer`}>{t.trading.buy}</button>
          <button onClick={() => setMode('sell')} className={`${mode === 'sell' ? 'bg-btn-sell text-white' : 'bg-gray-900 text-white'} py-2 rounded-md cursor-pointer`}>{t.trading.sell}</button>
        </div>
        <div className="inline-flex items-center bg-gray-500 gap-2 px-2 py-1 text-white/60 text-sm rounded-md cursor-pointer mb-1">
          <span>⚙️</span>
          <span className="text-xs text-white">{t.common.settings}</span>
        </div>
        ...
        <button className="w-full py-3 rounded-md font-semibold" disabled style={{ backgroundColor: "#CCAE00", opacity: amount.length > 0 ? "1" : ".6" }}>{t.common.connectWallet}</button>
      </div>
